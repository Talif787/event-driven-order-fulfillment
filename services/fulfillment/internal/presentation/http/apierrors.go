...
// writeError maps domain and application errors to a stable HTTP envelope and
...
func classify(err error) (int, string, string) {
	switch {
	case errors.Is(err, fulfillment.ErrValidation), errors.Is(err, fulfillment.ErrInvalidIdentifier):
		return http.StatusBadRequest, "VALIDATION_ERROR", err.Error()
	case errors.Is(err, fulfillment.ErrShipmentNotFound):
		return http.StatusNotFound, "NOT_FOUND", err.Error()
	case errors.Is(err, fulfillment.ErrShipmentExists):
		return http.StatusConflict, "CONFLICT", err.Error()
	case errors.Is(err, fulfillment.ErrConcurrency):
		return http.StatusConflict, "CONFLICT", "the shipment was modified concurrently, please retry"
	case errors.Is(err, fulfillment.ErrInvalidTransition):
