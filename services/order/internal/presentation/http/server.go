...
func Serve(ctx context.Context, srv *http.Server, shutdownTimeout config.TimeoutConfig) error {
	...
	case err := <-errCh:
		return err
	case <-ctx.Done():
		shutdownCtx, cancel := context.WithTimeout(context.Background(), shutdownTimeout.Shutdown)
		defer cancel()
